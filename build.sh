...
# The site is served from a subpath. Anything written in a template goes through
# relURL, but SVG generated by the pipeline is inlined verbatim — an absolute path
# in there 404s in production while working perfectly on a root-served dev server.
